...
> Navigation **structure** for the NS app: navigation region, top bar, tabs vs bottom nav, back navigation, loading transitions. For transient **surfaces/overlays** — bottom sheets, dialogs, alerts, action sheets, menus, focused flows, persistent panels — see `interaction-models.md` (this doc does not duplicate those decision rules). iOS-first; Android and iOS-26+ differences are marked. Source of truth: Nessie design system (design.ns.nl) + Notion App Guidelines. Never invent token or component names. Where the source is silent this is flagged — confirm with the Nessie team rather than inventing.
...
## Transient surfaces (sheets, dialogs, menus) → `interaction-models.md`
