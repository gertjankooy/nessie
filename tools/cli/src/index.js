...
const VENDOR = '.nessie';
const MD_TARGETS = ['CLAUDE.md', 'AGENTS.md', '.github/copilot-instructions.md'];
const CURSOR_RULE = '.cursor/rules/nessie.mdc';
...
export async function run(argv) {
  ...
  if (cmd === 'remove') return remove(flags);
...
async function selectTools(flags, dir) {
  if (flags.all) return TOOL_KEYS;
  if (flags.tools) return parseToolList(flags.tools);

  const detected = detectTools(dir);
  const fallback = detected.length ? detected : ['cursor', 'codex'];
  if (flags.yes || !process.stdin.isTTY) return fallback;

  const ans = await promptLine(`Which tools? (${TOOL_KEYS.join(', ')})`, fallback.join(','));
  const list = parseToolList(ans);
...
async function init(flags) {
  ...
  const tools = await selectTools(flags, dir);
  ...
  console.log(
    `\n${c.green('Done.')} Your agent reads ${c.bold(`${VENDOR}/AGENTS.md`)}. ` +
      `Check for newer docs with ${c.bold('npx nessie-skill status')}, refresh with ${c.bold('update')}.`
  );
}
...
async function remove(flags) {
  ...
  if (existsSync(dest)) {
    ...
    console.log(`${c.green('✓')} removed ${VENDOR}/`);
    any = true;
  }
  for (const rel of MD_TARGETS) {
    const res = await removeManagedBlock(join(dir, rel));
    if (res !== 'absent') {
      console.log(`${c.green('✓')} ${res} ${rel}`);
      any = true;
    }
  }
  const cursor = join(dir, CURSOR_RULE);
...
${c.bold('Usage')}
  ...
  npx nessie-skill status [options]   Show installed version and whether an update exists
  npx nessie-skill remove [options]   Remove .nessie/ and the entry-file blocks
  npx nessie-skill help

${c.bold('Options')}
  --tools <list>   Comma list: ${TOOL_KEYS.join(', ')} (default: auto-detect / prompt)
  --all            Configure every supported tool
  --yes            Non-interactive: accept detected tools, no prompt
...
${c.bold('Examples')}
  ...
  npx nessie-skill init --all --yes --ref v1.0.0
  npx nessie-skill status
